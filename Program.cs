...
Console.WriteLine("ingrese una operación (sin espacios): ");
cadeCat = Console.ReadLine().ToString();
if(cadeCat.Contains("+"))
{
    resul = 0;
    foreach(string num in cadeCat.Split("+"))
    {
        if (float.TryParse(num, out n1))
        {
            resul += n1;
        }
    }
}
else if(cadeCat.Contains("-"))
{
    resul = 0;
    foreach(string num in cadeCat.Split("-"))
    {
